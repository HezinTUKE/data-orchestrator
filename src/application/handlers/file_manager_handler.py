...
class FileManagerHandler:
    ...
    async def store_metadata(cls, file_content: bytes, file_name: str, overwrite: bool, session: AsyncSession) -> bool:
        ...
        if dc_model:
            _query = update(MetadataModel).filter(
                MetadataModel.file_metadata_id == dc_model.file_metadata_id
            ).values(
                updated_at = dc_model.updated_at
            )
    ...
    @classmethod
    async def process_files(cls, file_ids: str):
        ...

    @classmethod
    async def _overwrite_policy(cls, overwrite: bool, storage_path: str, file_name: str, session: AsyncSession) -> tuple[bool, MetadataBaseDC | None]:
        if not overwrite:
            _query = select(
                exists().where(MetadataModel.storage_path == storage_path, MetadataModel.file_name == file_name))
            _query_result = await session.execute(_query)
            exists_ = _query_result.scalar()
            return not exists_, None
        else:
            _query = select(MetadataModel).filter(MetadataModel.storage_path == storage_path,
