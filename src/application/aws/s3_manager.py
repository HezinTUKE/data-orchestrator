...
class StorageManager:
    ...
    def __init__(self):
        ...
        if not self._check_bucket_exists(
            bucket_name=bucket_name, s3_client=self.s3_client
        ):
            self.bucket = self.s3_client.create_bucket(
                Bucket=bucket_name,
                ACL="private",
                CreateBucketConfiguration={
                    "LocationConstraint": self.aws_config.get("location")
                },
            )
        else:
    ...
    def upload_file(self, file_path: str, file_bytes: bytes) -> bool:
        transfer_config = TransferConfig(
            multipart_threshold=CHUNK_SIZE, max_concurrency=10
        )
        ...
        try:
            transfer_manager.upload(
                BytesIO(file_bytes), bucket=self.bucket.name, key=file_path
            )
            return True
